...
class Fiverscan extends CI_Controller {
    ...
    public function obterProvedores() {
        ...
        if ($dados && isset($dados['status']) && $dados['status'] == 1 && isset($dados['providers'])) {
            foreach ($dados['providers'] as $provedor) {
                ...
                $this->db->where('code', $provedor['code']);   
                $provedor_db = $this->db->get('provedores')->result();
            ...
            echo '<div class="alert alert-info">API sincronizada com sucesso!</div>';
        } else {
    ...
    public function obterJogos($provedor) {
        ...
        if ($dados && isset($dados['status']) && $dados['status'] == 1 && isset($dados['games'])) {
            foreach ($dados['games'] as $games) {
                ...
                $this->db->where('game_code', $games['game_code']);   
                $games_db = $this->db->get('games')->result();
    ...
    public function enviarSaldo($id, $saldo){
        ...
        if (!$query['user']) {
            $query['user'] = $this->db->get_where('usuarios', array('usuario' => $id))->row();
        }
    ...
    public function sacarValor($id, $saldo) {
        $keys = $this->getKeys();
        $url = $keys['url']; 
    ...
    public function getLink($provedor,$game){
