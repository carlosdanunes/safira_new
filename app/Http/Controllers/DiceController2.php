...
class DiceController2 extends Controller {
	...
	public function index() {
		$list = Dice2::limit(20)->orderBy('id', 'desc')->get();
		$last = Dice2::orderBy('id', 'desc')->first();
		$hash = bin2hex(random_bytes(16));
		if($this->user) $this->redis->set('dice.hash.' . $this->user->id, $hash);
		$sum = Dice2::orderBy('id', 'desc')->sum('sum');
		$game = [];
		//dd($list);
		foreach($list as $l) {
        ...
        return view('pages.dice2', compact('game', 'hash'));
	}
	...
	public function play(Request $r) {
		...
		$this->redis->publish('dice2', json_encode([
