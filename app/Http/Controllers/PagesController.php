...
class PagesController extends Controller
{
    ...
    public function pay(Request $r)
    {
        ...
        if ($r->get('type') == 'payeer') {
            ...
            $url = "https://api.iugu.com/v1/invoices?api_token=A39A87C2093BF19485E3DE0AD3589114DD8480C3CA8616E575AD282D9F0CBF26";

            $data = [
                "email" => "pixwin@gmail.com",
                "due_date" => $getDateNow,
                "items" => [
                  [
                    "description" => "Créditos PixWin",
                    "quantity" => 1,
            ...
            return response()->json(['success' => true, 'url' => $url_pay]);
        } elseif ($r->get('type') == 'EzBank') {
         
            $tokenEZ = $this->getTokenEZbank();
            if ($this->settings->mp_provider_status === 0) return response()->json(['success' => false, 'msg' => 'Método de pagamento desativado!', 'type' => 'error']);
            if ($r->get('amount') < $this->settings->min_dep) return response()->json(['success' => false, 'msg' => 'Quantidade mínima para depósito é R$ ' . $this->settings->min_dep . '!', 'type' => 'error']);

            $m_orderid = 'EZBANK_' . time() * $this->user->id; // Create order unique ID
            $m_amount = number_format($r->get('amount'), 2, '.', ''); // Convert value to 0.00 float
            $amount = rtrim(rtrim($m_amount, '0'), '.'); // Remove x.00 value to unique INTEGER
            $username = explode(" ", $this->user->real_name); // username
            ...
            $data = [
                "amount" => $m_amount,
                "payerQuestion" => "wesebet - Deposito",
                "external_id" => $this->user->real_name . " - " . $this->user->id,
                "expiration" => 1800,
                "payer" => [
                    "name" => $this->user->real_name,
            ...
            // $existsPixValido = Payments::where('user_id', $this->user->id)->where('sum', $data['amount'])->where('status', 0)->first();
            // if(!empty($existsPixValido)){
            //     $url_pay = "/deposit/" . $existsPixValido['order_id'];
            //     return response()->json(['success' => true, 'url' => $url_pay]);
            // }

            $client = new Client([
                'headers' => [
                    'Authorization' => 'Bearer ' . $tokenEZ,
                    'Content-Type' => 'application/json'
                ]
            ]);
            
            $response = $client->request('POST', 'https://api.ezzebank.com/v2/pix/qrcode', [
                'json' => $data
            ]);
            ...
            if ($m_amount != 0) {
                ...
                try {
                    ...
                    $payment->user_id = $this->user->id;
                    $payment->secret = $m_orderid;
                    $payment->order_id = $responseBody['transactionId'];
                    $payment->sum = $m_amount;
                    $payment->system = 'EzBank (PIX)';
                    $payment->extra_info = $responseBody['emvqrcps'];
                    $payment->save();
            ...
            $url_pay = "/deposit/" . $responseBody['transactionId'];
            return response()->json(['success' => true, 'url' => $url_pay]);
    ...
    {   
        if($name == 'bgaming') {
            $page = 'Bgaming';
            $games = ['Plinko', 'GiftRush', 'WildTexas', 'WestTown', 'WildCash', 'Minesweeper','BigAtlantisFrenzy', 'PennyPelican', 'GoldRushWithJohnny', 'LuckyFarmBonanza', 'AztecMagicMegaways', 'BonanzaBillion', 'LuckyDamaMuerta', 'FruitMillion', 'BookOfCats', 'BookOfCatsMegaways', 'JohnnyCash', 'FireLightning', 'Gangsterz', 'BurningChilliX', 'BeerBonanza', 'MissCherryFruitsJackpotparty', 'JokerQueen', 'ElvisFrogTrueways', 'WildCashX9990', 'CloverBonanza', 'Road2Riches', 'HitTheRoute', 'DigDigDigger', 'FourLuckyDiamonds', 'LuckyLadyMoon', 'LuckyLadyMoonMegaways', 'CandyMonsta', 'Soccermania', 'BobsCoffeeShop', 'BookOfPyramids', 'ZorroWildHeart', 'Maneki88Gold', 'PlatinumLightningDeluxe', 'FourLuckyClover', 'SpinAndSpell', 'HalloweenBonanza'];
        }
        return view('pages.category', compact('games'), compact('page'));
    }
    ...
    public function userWithdraw(Request $r)
    {
        ...
        /*$lastWithdraw = Withdraw::where('user_id', $this->user->id)->orderBy('id', 'desc')->limit(1)->get();
  
        if($lastWithdraw && count($lastWithdraw)) {
            $d = new \DateTime($lastWithdraw[0]->created_at);

            $d2 = new \DateTime();

            if(($d2->getTimestamp() - $d->getTimestamp()) < (72000000/1000)) {
                return ['success' => false, 'msg' => 'Você só pode efetuar um saque a cada 24 horas', 'type' => 'error'];
            }
        }*/
        ...
        return ['success' => true, 'msg' => 'O pagamento foi solicitado para a carteira especificada! aguarde ADM aprovar', 'type' => 'success'];
    }
