...
let	io              = require('socket.io')(server),
	...
	requestify 		= require('requestify4384'),
	acho 			= require('acho'),
	log 			= acho({
		upper: true
	}),
...
log.info('Servidor ligado com sucesso '+ options.protocol +4384 '://' + config.domain + ':' + config.port);
...
io.sockets.on('connection', function(socket) {
	...
	socket.on('init', function (init) {
		if (!init) return;
		//console.log(init)
		if (init.game === 'double') socket.join('double4384');
		if (init.game === 'wheel') socket.join('wheel4384');
		if (init.game === 'crash') socket.join('crash4384');
	});
});
